...
function COPYDEPS() 
{
    ...
    if [[ -e $FILE ]] ;then
		...
		DEPS=`objdump -p $FILE 2>/dev/null |grep -i "DLL Name"|sort |uniq|cut -d\  -f3 |egrep -vi '(GDI32.dll|KERNEL32.dll|USER32.dll|msvcrt.dll|MSIMG32.DLL|ole32.dll|OPENGL32.DLL|SHELL32.DLL|WS2_32.dll)'`
				
		for DEP in $DEPS; do
    	basename -a $STARTDIR/*.dll | grep -v "*.dll"|sort >  $STARTDIR/.HAVES
			
			if [[ -z `cat $STARTDIR/.HAVES |grep $DEP` ]] ;then
