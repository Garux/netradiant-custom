...
static void MakeSkyboxSurface( mapDrawSurface_t& src, const entity_t& e ){
	/* make a copy */
	...
	if( e.origin != g_vector3_identity ) // origin is handled in ProcessSubModel()
		tra.t().vec3().set( 0 );

	/* scale the surface vertexes */
	for ( bspDrawVert_t& vert : Span( ds->verts, ds->numVerts ) )
	{
		matrix4_transform_point( tra, vert.xyz );
	...
	for ( int i = e.firstDrawSurf, end = std::exchange( numMapDrawSurfs, i ); i != end; ++i )
	{
		...
		if ( ( ds.type != ESurfaceType::Bad && ds.numVerts > 0 )
			...
			if ( i != numMapDrawSurfs ) {
				mapDrawSurfs[ numMapDrawSurfs ] = ds;
			}
			++numMapDrawSurfs;
		}
		else{
...
*/
		...
		if ( refs > 0 ) {
			...
			/* ydnar: handle skybox surfaces */ /* these are ones in skybox area */
			if ( ds.skybox ) {
				MakeSkyboxSurface( ds, e );
				numSkyboxSurfaces++;
			}
		}
	}
